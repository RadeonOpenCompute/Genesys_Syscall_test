...
static uint32_t faisal(uint32_t halfblock, uint64_t subkey, const uint8_t *sbox) [[hc]]
{
	uint64_t expanded = 0;
	for (int i = 0; i < 8; ++i)
...
static uint64_t run_des(uint64_t block, uint64_t key, const uint8_t *sbox) [[hc]]
{
	block = initial_perm(block);

	uint32_t a = block >> 32, b = block;
	for (int i = 0; i < 16; ++i) {
		uint32_t tmp = a ^ faisal(b, subkey(key, i), sbox);
		a = b;
		b = tmp;
	}
